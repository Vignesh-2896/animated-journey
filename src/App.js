...
function App() {
  ...
  useEffect(() => {
    (async () => {
      ...
      generationData.forEach(async function (generationEntry, index) {
        ...
        generationObject["region"] =
          generationInfo.main_region["name"].toUpperCase();

        let temp = generationList;
        temp.splice(index, 0, generationObject);
        setGenerationList(temp);
      });
  ...
  const updateTypeData = async (e) => {
    ...
    */
    ...
    let tempData = filterPokemonTypeList;
    tempData.push(e.target.value);
    setFilterPokemonTypeList(tempData);
    filterPokemon();
  };
  ...
  const filterPokemon = () => {
    ...
    pokemonList.forEach(async function (pokemonEntry) {
      ...
      pokemonTypes.forEach(function (pokemonTypeItem) {
        if (
          filterPokemonTypeList.indexOf(pokemonTypeItem.type["name"]) !== -1
        ) {
  ...
  const updatePokemonData = async (e) => {
    ...
    generationData.forEach(async function (generationItem) {
      let pokemonData;
      if (pokemonAlteredName[generationItem.name])
        pokemonData = await fetchPokemonData(
          pokemonAlteredName[generationItem.name]
        );
                  ...
                  return (
                    <div className="filterTypes-item" key={item}>
                      <input
                        ...
                        value={item}
                      />
                      <label className={item} htmlFor={`${item}_type`}>
                        {capitalize(item)}
                      </label>
                    </div>
                  );
...
const fetchPokemonData = async (pokedexEntry) => {
  let response = await fetch(
    `https://pokeapi.co/api/v2/pokemon/${pokedexEntry}/`
  );
  response = await response.json();
  return response;
};
